...
fun <S,T> Promise<T>.catchAxiosError(onRejected: (AxiosError) -> S): Promise<S> = catch { e ->
	val error = e.axiosError
	onRejected(error)
}

internal val Throwable.axiosError get() = asDynamic().unsafeCast<AxiosError>()
